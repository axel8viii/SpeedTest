...
public:
    ...
    const long &latency();
    bool downloadSpeed(const ServerInfo& server, const TestConfig& config, double& speed, long& size, std::function<void(bool)> cb = nullptr);
    bool uploadSpeed(const ServerInfo& server, const TestConfig& config, double& speed, long& size, std::function<void(bool)> cb = nullptr);
...
private:
    ...
    std::pair<double, long> execute(const ServerInfo &server, const TestConfig &config, const opFn &fnc, std::function<void(bool)> cb = nullptr);
    template <typename T>
    ...
    long mLatency;
    std::pair<double, long> mUploadSpeed;
    std::pair<double, long> mDownloadSpeed;
